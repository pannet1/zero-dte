...
from utilities.utils import adjust_ltp
from utilities.regative import Regative
...
class Mcx:
    ...
    def close_positions(cls, row):
        try:
            ...
            prc = adjust_ltp(row["last_price"], dir *
                             cls.smcx["BUFF_PERC"], row["ti"])
            # prc = row['last_price'] + \
            ...
            if resp:
                ...
                logging.warning(
                    f"closing position {args} with tick size: {row['ti']}")
        except Exception as e:
